...
from train_addingnpy import train_addingnpy
...
def run_train(config, config_id):
    ...
    #for batch in train_loader:
     #   np.save(f"local_directory/npy_train_image/{global_index}", batch["image"])
      #  np.save(f"local_directory/npy_train_label/{global_index}", batch["label"])
       # print(f"saving traning files {global_index}...")
        #global_index+=1
    #print(f"Training sample size: {global_index+1}")
    val_loader = val_dataloader(config, val_images)
    #global_index = 0
    # for batch in val_loader:
    #     np.save(f"local_directory/npy_val_image/{global_index}", batch["image"])
    #     np.save(f"local_directory/npy_val_label/{global_index}", batch["label"])
    #     print(f"saving validation files {global_index}...")
    #     global_index += 1
    # print(f"Validation sample size: {global_index+1}")
...
if __name__ == "__main__":
    ...
    CONFIG_DIR = "home/local/VANDERBILT/dongc1/lobe_seg/configs"
    config = load_config(f"{args.config_id}.yaml", CONFIG_DIR)
