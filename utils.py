...
def get_registration_command_non_rigid(registration_method_name,
    ...
    if registration_method_name == 'deformable_deedsBCV':
        ...
        command_list.append(
            f'{deedsBCVslow_path} {reg_args} -F {fixed_image} -M {moving_image} -O {output_image} -A {actual_output_mat_path} {label_prop_command}')
        ...
        command_list.append(
            f'{deedsBCV_path} {reg_args} -F {fixed_image} -M {moving_image} -O {output_image} -A {actual_output_mat_path} {label_prop_command}')
...
def get_registration_command(registration_method_name, reg_args, label_file, reg_tool_root, fixed_image, moving_image,
                             output_image, output_mat):
    command_list = []
    actual_output_mat_path = output_mat + '_matrix.txt'
    ...
    if registration_method_name == 'affine_flirt':
        ...
        command_list.append(
            f'{flirt_path} -v -dof 6 -in {moving_image} -ref {fixed_image} -omat {mid_step_rigid_mat} -out {mid_step_rigid_im} -nosearch')
        # 2. DOF 9 Affine.
        command_list.append(
            f'{flirt_path} -v -dof 9 -in {moving_image} -ref {fixed_image} -init {mid_step_rigid_mat} -omat {output_mat} -out {output_image} -nosearch')
    elif registration_method_name == 'affine_nifty_reg':
        reg_aladin_path = os.path.join(reg_tool_root, 'reg_aladin')
        output_mat_real = output_mat.replace('.nii.gz', '.txt')
        command_list.append(
            f'{reg_aladin_path} -ln 5 -ref {fixed_image} -flo {moving_image} -res {output_image} -aff {output_mat_real}')
        ...
        command_list.append(
            f'{reg_aladin_path} -ln 5 -ref {fixed_image} -rmask {fixed_image_mask} -flo {moving_image} -fmask {moving_image_mask} -res {output_image} -aff {output_mat_real}')
        ...
        command_list.append(f'{linearBCVslow_path} -F {fixed_image} -M {moving_image} -O {output_mat}')
        command_list.append(
            f'{applyLinearBCVfloat_path} -M {moving_image} -A {actual_output_mat_path} -D {output_image}')
