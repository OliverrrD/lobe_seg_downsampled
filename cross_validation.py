...
import MetricLogger
from dataloader import npy_train_loader, npy_val_loader, npy_test_loader, train_dataloader, val_dataloader
...
def train_one_fold(config, config_id, k):
    ...
    images = [x for i, x in enumerate(folds) if i != (k - 1)]  # all folds excep
    ...
    val_loader = npy_val_loader(config, val_images)
...
def test_one_fold(config, config_id, out_name, k, output_seg=True, output_clip=True):
    ...
    images = images[k-1]
    test_loader = npy_test_loader(config, images)
