...
def npy_train_loader(config, npys):
    ...
    data = [os.path.join(data_dir, os.path.basename(npy)) for npy in npys]
    labels = [os.path.join(label_dir, os.path.basename(npy)) for npy in npys]
    files = [
        {"image": npy, "label": label}
        for npy, label in zip(data, labels)
    ]
...
def npy_train_loader_binary(config, npys):
    label_dir = config["label_dir"]
    data_dir=config["data_dir"]
    data = [os.path.join(data_dir, os.path.basename(npy)) for npy in npys]
    labels = [os.path.join(label_dir, os.path.basename(npy)) for npy in npys]
...
def npy_test_loader_binary(config, npys):
    label_dir = config["test_label_dir"]
    data_dir = config["test_dir"]
    data = [os.path.join(data_dir, os.path.basename(npy)) for npy in npys]
    labels = [os.path.join(label_dir, os.path.basename(npy)) for npy in npys]
...
def npy_val_loader(config, npys):
    label_dir = config["label_dir"]
    data_dir = config["data_dir"]
    data = [os.path.join(data_dir, os.path.basename(npy)) for npy in npys]
    labels = [os.path.join(label_dir, os.path.basename(npy)) for npy in npys]
...
def npy_val_loader_binary(config, npys):
    label_dir = config["label_dir"]
    data_dir = config["data_dir"]
    data = [os.path.join(data_dir, os.path.basename(npy)) for npy in npys]
    labels = [os.path.join(label_dir, os.path.basename(npy)) for npy in npys]
...
def infer_dataloader_binary(config, val_images):
    test_transforms = Compose([
        LoadImaged(keys=["image"]),
        AddChanneld(keys=["image"]),
        Spacingd(keys=["image"], pixdim=config["pix_dim"], mode=("bilinear")),
