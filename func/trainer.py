from func.data.data_generator import CNNDataset, CNNDiag, thorax_loader
import os
...
class Trainer(object):
    ...
    def data_loader(self):  
        ...
        if self.cfig['loader_name'] == 'CNNDataset':
            training_set=CNNDataset(partition_IDs['train'], dict_paths, path_labels,  sample_size, argu = True, use_mask = self.cfig['use_mask'], lastscan = self.cfig['lastscan'])
            validation_set=CNNDataset(partition_IDs['validation'], dict_paths, path_labels, sample_size, argu = False, use_mask = self.cfig['use_mask'], lastscan = self.cfig['lastscan'])
            test_set = CNNDataset(partition_IDs['test'], dict_paths, path_labels, sample_size, argu = False, use_mask = self.cfig['use_mask'], lastscan = self.cfig['lastscan'])
        if self.cfig['loader_name'] == 'CNNDiag':
            training_set=CNNDiag(partition_IDs['train'], dict_paths, dict_diags, labels, path_labels, sample_size, argu = True, use_mask = self.cfig['use_mask'], lastscan = self.cfig['lastscan'])
            validation_set=CNNDiag(partition_IDs['validation'], dict_paths, dict_diags, labels, path_labels,sample_size, argu = False, use_mask = self.cfig['use_mask'], lastscan = self.cfig['lastscan'])
            test_set=CNNDiag(partition_IDs['test'], dict_paths, dict_diags, labels, path_labels, sample_size, argu = False, use_mask = self.cfig['use_mask'], lastscan = self.cfig['lastscan'])
        if self.cfig['loader_name'] == 'thorax_loader':
            training_set = thorax_loader(partition_IDs['train'], dict_paths, path_labels, sample_size, argu=True, lastscan=self.cfig['lastscan'], use_mask = self.cfig['use_mask'])
            validation_set = thorax_loader(partition_IDs['validation'], dict_paths, path_labels, sample_size, argu=False,
                                         lastscan=self.cfig['lastscan'], use_mask = self.cfig['use_mask'])
            test_set = thorax_loader(partition_IDs['test'], dict_paths, path_labels, sample_size, argu=False,
                                   lastscan=self.cfig['lastscan'], use_mask = self.cfig['use_mask'])
        ...
        for batch_idx, data_tup in enumerate(self.train_loader):
            ...
            if self.disbce == True:
                ...
                if self.add_reg == False:
                    ...
                    print (scan_t.shape, diag.shape)
                    assert self.cfig['loss_name'] == 'reg_bl' 
                    regloss = LossPool(pred, y_reg, self.cfig, scan_t = scan_t, diag_t = diag, loss_name = self.cfig['loss_name']).get_loss()
                    celoss = LossPool(pred, target, self.cfig, loss_name='cross_entropy_loss').get_loss() # compare bi_ce_loss
                    loss = self.cfig['alpha'] * regloss + celoss
                    #loss = regloss 
                    print ('reg loss: ', regloss.data.cpu().numpy(), 'celoss: ',celoss.data.cpu().numpy(), 'total loss', loss.data.cpu().numpy())
            ...
            if self.add_reg: regloss_list.append(regloss.data.cpu().numpy().tolist())
            #break
    ...
    def test_gcam(self, epoch, save_path, target_layer = 'ec1.2'):    # not finished.
        self.model.eval()
        model_root = osp.join(self.cfig['save_path'], 'models')
        model_pth = '%s/model_epoch_%04d.pth' % (model_root, epoch)
        if self.device == 'cuda': #there is a GPU device
        ...
        with torch.no_grad():
            for batch_idx, data_tup in enumerate(loader):
                ...
                if self.add_reg == False:
                    ...
                    output_img_file = os.path.join(out, ('%s_img.nii.gz' % ID))
                    nib.save(nii_img, output_img_file)
                    nii_mask = nib.Nifti1Image(input_mask, affine=np.eye(4))
                    output_mask_file = os.path.join(out, ('%s_mask.nii.gz' % ID))
        ...
        #target_layer = 'ec6.2'
        # target_layer = 'ec1.2'
        test_attention_out = os.path.join(out, target_layer)
        ...
        input_mask = data[0, 1].data.cpu().numpy()
        nii_img = nib.Nifti1Image(input_img, affine=np.eye(4))
        output_img_file = os.path.join(out, ('%s_img.nii.gz' % os.path.basename(sub_name[0])))
        nib.save(nii_img, output_img_file)
        nii_mask = nib.Nifti1Image(input_mask, affine=np.eye(4))
        output_mask_file = os.path.join(out, ('%s_mask.nii.gz' % os.path.basename(sub_name[0])))
        nib.save(nii_mask, output_mask_file)
        del input_img
        del nii_img
        ...
        for i in range(0, topk):
            ...
            output_att_file = os.path.join(test_attention_out, ('%s_test_att%d_clss%d.nii.gz' % (os.path.basename(sub_name[0]),i,idx[i])))
        ...
        for subi in range(subnum):
            ...
            output_att_file = os.path.join(out, ('%s_train_att.nii.gz' % os.path.basename(sub_name[subi])))
            nib.save(nii_seg, output_att_file)
            del nii_seg, fmap, attmap, attention
        # find details at /Users/riqianggao/MASI/DeepLungNet/YK/torchsrc/trainer.py
