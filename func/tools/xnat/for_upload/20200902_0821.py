...
data_root = '/nfs/masi/MCL/xnat/xnat20200902_original/08-21-2020'
subj_list = os.listdir(data_root)
for subj in subj_list:
    if not os.path.isdir(data_root + '/' + subj): 
        continue
    
    #assert len(os.listdir(data_root + '/' + subj)) == 1
    if os.path.exists(data_root + '/'  + subj + '/DICOM'):
        subj_path = data_root + '/'  + subj + '/DICOM'
    else:
        subj_path = data_root + '/'  + subj + '/' + subj +  '/DICOM'
    ...
    for sess in sess_list:
        if sess[0] == '.':
            continue
        dcm_list = glob(os.path.join(subj_path, sess, '*'))
        
        print (len(os.listdir(data_root + '/' + subj)) , subj, sess)
...
data.to_csv('/nfs/masi/MCL/xnat/xnat20200902_original/0821upload.csv', index=False)