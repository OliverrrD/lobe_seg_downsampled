...
data_root = '/nfs/masi/MCL/xnat/xnat20201031/UCMC'
subj_list = os.listdir(data_root)

for subj in subj_list:
    try: 
        sub1folds = os.listdir(data_root + '/' + subj)
    except:
        continue
    for sub1 in sub1folds:
        sub2folds = os.listdir(data_root + '/' + subj + '/' + sub1)
        for sub2 in sub2folds:
            dcm_list = glob(os.path.join(data_root + '/' + subj + '/' + sub1, sub2, '*.dcm'))
...
data.to_csv('/nfs/masi/MCL/xnat/xnat20201031/upload.csv', index=False)