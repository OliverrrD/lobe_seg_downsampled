...
import MetricLogger
from dataloader import  npy_train_loader_binary,npy_test_loader_binary, npy_val_loader_binary
from models import unet256, unet512, unet1024, unet64
...
def run_train(config, config_id):
    ...
    # get dataloaders
    train_loader = npy_train_loader_binary(config, train_images)
    val_loader = npy_val_loader_binary(config, val_images)
...
def run_test(config, config_id, out_name, output_seg=False, output_clip=False):
    ...
    # Load data
    images = sorted(glob.glob(os.path.join(DATA_DIR, config["image_type"])))
    test_loader = npy_test_loader_binary(config, images)
...
if __name__ == "__main__":
    ...
    CONFIG_DIR = "/home-nfs2/local/VANDERBILT/dongc1/lobe_seg/configs"
